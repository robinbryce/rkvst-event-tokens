...
import fs from "fs";
import { DiamondDeployer, FacetCutOpts, FileReader, Reporter } from "@polysensus/diamond-deploy";

const DIAMOND_DEPLOY_JSON = process.env.DIAMOND_DEPLOY_JSON ?? 'dist/diamond-deploy.json';
...
export async function deployRKVSTEventTokens(signer, owner, options={}) {
  ...
  if (!deployer.canDeploy())
    throw new Error(`can't deploy contracts, probably missing artifacts or facets`);
